...
class SRPlayer:
    ...
    def create_podcast_list(self):
        ...
        # Scroll window
        start_idx = max(0, self.selected_podcast_index - 15)
        end_idx = start_idx + 30
    ...
    def create_episode_list(self):
        ...
        # Scroll window
        start_idx = max(0, self.selected_episode_index - 10)
    ...
    def create_now_playing(self):
        ...
        if self.mode == 'podcast' and self.playing_episode:
            ...
            lines.append(Text(title, style=f"bold {LIGHT_TEXT} underline"))
            lines.append(Text(""))
            
            program = self.playing_episode.get('program', {})
            program_name = program.get('name', 'Unknown') if isinstance(program, dict) else 'Unknown'
            lines.append(Text("Podcast", style=GRAY_TEXT))
            lines.append(Text(program_name, style=LIGHT_TEXT))
            lines.append(Text(""))
            lines.append(Text(""))
            ...
            content = Align.center(Text("\n").join(lines), vertical="middle")
        
        # Radio mode
        elif self.mode == 'radio' and self.playing_channel:
            lines = []
            lines.append(Text(self.playing_channel['name'], style=f"bold {LIGHT_TEXT} underline"))
            lines.append(Text(""))
            lines.append(Text(""))
    ...
    def create_layout(self):
        ...
        layout["header"].update(Panel(Align.center(header_text), style=f"on {DARK_BG}"))
